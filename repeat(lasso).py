...
def Gibbs_MDGDP(n,R,p1,p2,p3,D,tau,sigma_real,M,burn):
    ...
    for d in range(D):
        ...
        for i in range(pd[d]):
            ...
            ei.append(list(np.ones(R)))
            fi.append(list(np.ones(R)))
    ...
    while T < M:
        ...
        for d in range(D):
            ...
            for i_d in range(pd[d]):
                ...
                zeta_id = math.log(1 - rho) - math.log(rho) + 0.5* math.log(Lambda_xing) - \
        ...
        for dd in range(D):
            ...
            #s_q_zjdr = s_q_zjdr/3;
        iteration_vartheta = float(np.random.gamma(shape = a_tau + R*(p1+p2+p3), scale = 1/(b_tau +s_h2/2),  size = 1));
        rho = beta.rvs(a_rho + s_gamma ,b_rho + (p1 + p2 +p3) - s_gamma,size = 1 );
